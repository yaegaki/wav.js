...
var wav = (function(){
	...
	function readData(reader, chunk, fmtChunk){
		if(fmtChunk){
			if(fmtChunk.bitPerSample !== 16 && fmtChunk.bitPerSample !== 8){
				...
				chunk.left = new dataArray(sampleLength);
				chunk.right = (fmtChunk.channels !== 1)? new dataArray(sampleLength): null;
	...
	*/
	function addWav(pos, targetWav){
		...
		var offset = Math.floor((pos * fmt.channels * fmt.samplingRate * (fmt.bitPerSample / 8)) / 1000) & 0xfffffffe;

		for(var i = 0;i < targetWav.chunks.data.sampleNum;i++){
			if((i+offset) >= this.chunks.data.sampleNum){
				break;
			}
			this.chunks.data.left[i+offset] += targetWav.chunks.data.left[i];
			if(flag){
				this.chunks.data.right[i+offset] += targetWav.chunks.data.right[i];
			}
		}
	}
...
var createWav = (function(){
	function createWav(samplingRate, millisec){
		...
		var data = {
			...
			left:new Int16Array(dataSize / 2),
			right:null
		};
		...
		if(data instanceof wav){
			...
			for(var i = 0;i < d.sampleNum;i++){
				if(fmt.bitPerSample === 16){
					write(Int16ToBytes(d.left[i]));
				}else{
					write(Uint8ToBytes(d.left[i]));
				}
				if(fmt.channels !== 1){
					if(fmt.bitPerSample === 16){
						write(Int16ToBytes(d.right[i]));
					}else{
						write(Uint8ToBytes(d.right[i]));
					}
				}
			}
